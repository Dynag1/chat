...
reportBtn.addEventListener('click', () => {
    ...
    // Save chat_id before it gets cleared
    const chatIdToReport = currentChatId;
    const partnerIdToReport = partnerId;
    ...
    fetch('api/action.php?action=report', {
        ...
        body: JSON.stringify({
            reported_id: partnerIdToReport,
            reason: reason,
            chat_id: chatIdToReport,
            csrf_token: csrfToken
        })
    })
        .then(res => res.json())
        .then(data => {
            if (data.success) {
                ...
                messagesDiv.innerHTML += '<div class="system-message">Utilisateur signalé. La conversation a été enregistrée pour examen par les modérateurs.</div>';
                messagesDiv.scrollTop = messagesDiv.scrollHeight;
            } else {
